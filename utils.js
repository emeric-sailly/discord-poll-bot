...
export async function DeleteAllChannelMessages(channelId) {
  ...
  await DiscordRequest(endpoint, {method: "POST", body: {messages: [2,100] }});
}

export async function GetChannelMessages(channelId) {
  const endpoint = `channels/${channelId}/messages?limit=2`;
  const res = await DiscordRequest(endpoint, {method: "GET"});
  const data = await res.json();
  console.log(data);
}

export async function DeleteAllCommands(appId, commandIDs) {
  commandIDs.forEach(async (commandID) => {
